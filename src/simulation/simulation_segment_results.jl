...
"""
Handles merging of simulation segments
"""
struct SimulationSegmentResults
    "Directory of main simulation"
    path::String
    "User-defined simulation name"
    simulation_name::String
    "Defines how the simulation is split into segments"
    segments::SimulationSegments
    "Cache of datasets"
    datasets::Dict{String, HDF5.Dataset}
end

function SimulationSegmentResults(path::AbstractString)
    config_file = joinpath(path, "simulation_segments", "config.json")
    config = open(config_file, "r") do io
        JSON3.read(io, Dict)
    end
    segments = IS.deserialize(SimulationSegments, config)
    return SimulationSegmentResults(
        path,
        basename(path),
        segments,
...
"""
Combine all segment simulation files.
"""
function join_simulation(path::AbstractString)
    results = SimulationSegmentResults(path)
    join_simulation(results)
    return
end

function join_simulation(results::SimulationSegmentResults)
...
_segment_path(x::SimulationSegmentResults, i) =
    joinpath(x.path, "simulation_segments", string(i), x.simulation_name)
_store_subpath() = joinpath("data_store", "simulation_store.h5")
_store_path(x::SimulationSegmentResults) = joinpath(x.path, _store_subpath())

function _check_jobs(results::SimulationSegmentResults)
    overall_status = RunStatus.SUCCESSFUL
    for i in 1:get_num_segments(results.segments)
        job_results_path = joinpath(_segment_path(results, 1), "results")
        status = deserialize_status(job_results_path)
        if status != RunStatus.SUCCESSFUL
            @warn "segment job index = $i was not successful: $status"
...
function _merge_store_files!(results::SimulationSegmentResults)
    HDF5.h5open(_store_path(results), "r+") do dst
        for i in 1:get_num_segments(results.segments)
            HDF5.h5open(joinpath(_segment_path(results, i), _store_subpath()), "r") do src
...
function _copy_datasets!(
    results::SimulationSegmentResults,
    index::Int,
    src::HDF5.File,
    dst::HDF5.File,
)
...
function _merge_dataset_columns!(results::SimulationSegmentResults, index, src, dst)
    num_columns = size(src)[1]
    step_range = get_absolute_step_range(results.segments, index)
    IS.@assert_op num_columns % length(step_range) == 0
    num_columns_per_step = num_columns ÷ length(step_range)
    skip_offset = get_valid_step_offset(results.segments, index) - 1
    src_start = 1 + num_columns_per_step * skip_offset
    len = get_valid_step_length(results.segments, index) * num_columns_per_step
...
function _merge_dataset_rows!(results::SimulationSegmentResults, index, src, dst)
    ...
    len = get_valid_step_length(results.segments, index) * num_rows_per_step
    src_end = src_start + len - 1
...
function _complete(results::SimulationSegmentResults, status)
